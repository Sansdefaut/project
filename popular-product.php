...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div class="text-end items-end flex flex-col justify-end">
        <h2 class="text-3xl font-bold leading-tight text-gray-900">Popular Product</h2>
        <div class="mt-4 mb-12 h-1 w-24 bg-black ite"> </div>
      </div>
     
     <div class="row">
     	<?php
    // Fetch popular products (by quantity desc, you can adjust logic as needed)
    $stmt = $connect->prepare("SELECT * FROM products WHERE status = 'active' ORDER BY product_quantity DESC, date_added DESC LIMIT 12");
    $stmt->execute();
