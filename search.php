...
?>
<body>
<?php include 'cart.php'; include 'sidebar.php'; ?>
<main class="py-5">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="text-start">
      ...
      <div class="mt-4 mb-12 h-1 w-24 bg-black"></div>
    </div>
    <div class="row">
      <?php
      if (isset($_GET['q']) && !empty(trim($_GET['q']))) {
        $q = '%' . trim($_GET['q']) . '%';
        $stmt = $connect->prepare("SELECT * FROM products WHERE status = 'active' AND (product_name LIKE ? OR product_description LIKE ?) ORDER BY date_added DESC");
        $stmt->execute([$q, $q]);
      } else {
        // Show all products if no search query
        $stmt = $connect->prepare("SELECT * FROM products WHERE status = 'active' ORDER BY date_added DESC");
        $stmt->execute();
      }
      if ($stmt->rowCount() > 0) {
        ...
        <a href="product-detail?productid=<?php echo $fetch_product['productId']; ?>">
          <div class="app-card app-card-doc shadow-sm h-100 bg-transparent">
            ...
            <div class="app-card-body p-3 has-card-actions flex-col flex gap-2">
              ...
              <h4 class="app-doc-title mb-0 font-extralight truncate"><?php echo htmlspecialchars($fetch_product['product_description']); ?></h4>
