...
$select_allorders = "SELECT * FROM sales WHERE salesId='$orderid' AND userId='".$_SESSION['userId']."'";
...
<div class="p-6 shadow-lg mb-5 bg-white rounded-lg">
                            ...
                            <div class="mt-6">
                                ...
                                <div class="flex justify-between items-center mt-2">
                                  ...
                                  <div class="flex justify-between items-center mt-2">
                                    <span class="text-sm text-gray-600">Deliverly Code:</span>
                                    <span class="font-semibold"><?php echo $fetch_delivery['code_now']; ?></span>
                                </div>
...
<script>
  ...
  $(document).on('submit', '#PayMentForm', function(event2) {
        ...
        if (PayMentGateWay != '' && paymentphpone != '' && salesId != '') {
            ...
            $.ajax({
                url: "http://localhost/newstore/customer/include/payment-html",
                method: 'POST',
        ...
        if (confirm("Are you sure you want to Cancel this sale now?")) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.error) {
                        $('#alert_message_danger').show().html(data.message);
                        $('#alert_message_success').hide();
                        $('#alert_message_warning').hide();
                    } else {
                        $('#alert_message_success').show().html(data.message);
                        $('#alert_message_warning').hide();
                        $('#alert_message_danger').hide();
                ...
                error: function() {
                    // Handle AJAX error
                    $('#alert_message_danger').show().html("An error occurred. Please try again.");
                    $('#alert_message_success').hide();
                    $('#alert_message_warning').hide();
                },
                complete: function() {
